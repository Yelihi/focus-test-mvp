...
### 배경 처리 (Background Replacement)

GoalSettingModal과 세션 중 설정 팝업에서 배경을 선택할 수 있다.

| 모드 | 동작 |
|------|------|
| 없음 | raw 카메라 영상 그대로 표시 (세그멘터 비활성화) |
| 블러 | `blur(12px)` 필터 배경 + 인물 마스크 합성 |
| 이미지 | 프리셋 사진 배경 + 인물 마스크 합성 |

선택 UI는 썸네일 그리드로 구성되며, 모달 프리뷰와 세션 화면 모두 실시간으로 전환된다. PiP 창에도 배경 처리 결과가 반영된다.
...
│   │   └── lib/
│   │       ├── setupAutoPip.ts
│   │       ├── pipCanvasRenderer.ts  # getSource() 콜백 — blur canvas/video 동적 전환
│   │       ├── manualPipFallback.ts
│   │       └── initPip.ts
│   │
│   ├── background-blur/          # 배경 처리 (블러 / 이미지 교체)
│   │   ├── model/types.ts        # BackgroundMode, BackgroundPreset
│   │   ├── config/presets.ts     # BACKGROUND_PRESETS (없음/블러/도서관/카페/숲/바다)
│   │   └── lib/
│   │       ├── backgroundBlurRenderer.ts  # Selfie Segmentation + Canvas 합성
│   │       ├── createBlurSession.ts
...
### 배경 처리 (Selfie Segmentation)

MediaPipe Selfie Segmentation으로 인물 영역을 분리하고, 배경 레이어와 합성하여 Canvas에 출력한다.

**합성 파이프라인**:

```
video → segmenter(15fps) → confidenceMask (Float32Array)
  ↓
배경 레이어 (blur 필터 or 이미지)  ←─ backgroundBlurRenderer
  +
personCanvas (video + destination-in mask) ──→ ctx.drawImage → 출력 canvas
```

**배경 레이어 분기** (`backgroundBlurRenderer.ts` 78–90줄):

```typescript
if (backgroundImage) {
  ctx.drawImage(backgroundImage, 0, 0, w, h);   // 이미지 배경
} else {
  ctx.save();
  ctx.filter = "blur(12px)";
  ctx.translate(w, 0);
  ctx.scale(-1, 1);
  ctx.drawImage(video, 0, 0, w, h);             // 블러 배경
  ctx.restore();
  ctx.filter = "none";
}
// 이후 personCanvas를 destination-in으로 마스킹 → 합성 (변경 없음)
```

**BackgroundMode 타입** (`model/types.ts`):
...
- `none` → 세그멘터 비활성화, raw 비디오 표시
- `blur` → `setBackgroundImage(null)` + 기존 blur 로직
- `image` → `loadBackgroundImage(src)` 후 `setBackgroundImage(img)`

**프리셋 이미지**: `public/backgrounds/` — `library`, `cafe`, `forest`, `ocean` (640×480 + 100×75 썸네일)
...
```
getSource() → canvas.drawImage(소스에 따라 미러링 조건부) → 상태 뱃지 오버레이
...
function render() {
  ...
  if (source instanceof HTMLCanvasElement) {
    // blur canvas는 이미 미러링된 상태 — 그대로 그림
...
`getSource`는 `SessionPage`에서 전달하는 콜백으로, 배경 처리가 켜져 있으면 blur canvas를, 아니면 raw video를 반환한다. `backgroundBlurEnabledRef`(ref 미러링)를 사용해 매 프레임 호출에서 최신 상태를 읽는다.
...
### 이슈 11: 배경 처리 활성화 시 PiP에 raw 비디오가 표시됨

**문제**: 배경 블러·이미지를 선택해도 PiP 창에는 배경 처리가 적용되지 않은 원본 카메라 영상이 표시됨.

**원인**: `pipCanvasRenderer`가 `sourceVideo: HTMLVideoElement`를 고정 참조로 받아 생성 시점에 바인딩되어 있었음. 배경 처리 결과는 별도 `<canvas>`에 렌더링되는데, PiP 렌더러는 이를 인지하지 못하고 항상 raw video를 읽음.

**해결**: `sourceVideo` 고정값 → `getSource: () => HTMLVideoElement | HTMLCanvasElement` 콜백으로 교체.

```
initPip(video, getBlurSource) → pipCanvasRenderer(getSource)
  render() 호출마다: getSource() → blur canvas (배경 활성) or video (배경 없음)
```

- blur canvas는 이미 좌우반전된 상태이므로 `instanceof HTMLCanvasElement`이면 미러 변환 생략
- `SessionPage`에서 `backgroundBlurEnabledRef`(ref 미러링)를 사용해 stable한 `getBlurSource` 콜백을 구성, 렌더 사이클마다 최신 상태 반영

---

### 이슈 12: 배경 이미지 프리셋이 단색으로 표시됨

**문제**: placeholder 이미지를 `sharp`로 solid color로 생성하여 실제 배경처럼 보이지 않음.

**해결**: picsum.photos에서 seed 기반 무료 사진을 다운로드하여 교체.

```bash
curl -sL "https://picsum.photos/seed/library/640/480" -o public/backgrounds/library.jpg
# cafe, forest, ocean 동일 방식
```

seed 기반이므로 매 빌드에서 동일한 이미지가 사용된다. 실제 주제에 맞는 사진으로 교체하려면 같은 경로에 덮어쓰면 된다.
...
- **배경 이미지**: 현재 picsum.photos placeholder — 실제 주제에 맞는 사진으로 교체 예정
- **3D 아바타**: 다음 PR 예정 (VRoid VRM + Three.js)
